...
def filter_headers(headers):
    excluded_headers = ["content-encoding", "content-length", "transfer-encoding", "connection", "content-security-policy", "alt-svc"]
...
def handle_preflight():
    if request.method == "OPTIONS":
        ...
        if origin:
            ...
            response.headers["Access-Control-Allow-Origin"] = f"http://{request.host}"
...
def build_response(resp):
    ...
    if "text/html" in content_type:
        ...
        error_div = soup.new_tag("div", style=(
        ...
        h1 = soup.new_tag("h1")
        h1.string = "Please enable JavaScript"
        p = soup.new_tag("p")
        p.string = "This website requires JavaScript to function properly. Please enable JavaScript in your browser settings and reload the page."
        error_div.append(h1)
        error_div.append(soup.new_tag("br"))
        error_div.append(p)
        noscript_tag.append(error_div)
        if soup.body:
            soup.body.insert(0, noscript_tag)
        elif soup.html:
            soup.html.append(noscript_tag)
        def rewrite_url(url):
    ...
    if 'set-cookie' in resp.headers:
        raw_cookies = resp.headers.getlist('set-cookie') if hasattr(resp.headers, 'getlist') else [resp.headers['set-cookie']]
